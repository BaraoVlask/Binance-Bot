                                    ...
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            @php
                                                ...
                                                if($data->{$row->field} instanceof \stdClass){
                                                    $data->{$row->field} = json_encode($data->{$row->field});
                                                }
                                            @endphp
                                            <td>
                                                ...
                                                @elseif($row->type == 'date' || $row->type == 'timestamp')
                                                    ...
                                                    @else
                                                        {{ $data->{$row->field} }}
                                                    @endif
                                                @elseif($row->type == 'text' && $data->{$row->field} instanceof UnitEnum)
                                                    @include('voyager::multilingual.input-hidden-bread-browse')
                                                    <div>{{ mb_strlen( $data->{$row->field}->value ) > 200 ? mb_substr($data->{$row->field}->value, 0, 200) . ' ...' : $data->{$row->field}->value }}</div>
